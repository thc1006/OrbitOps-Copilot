...
**What's still pending for the RunSpace submission**: pitch-deck PDF render, demo video recording (90 s + 3 min), and the Sprint-3 CesiumJS satellite-pass viz (VS-13). Phase H UI completeness sweep landed in PRs #45–#46 (2026-05-02): Beams elevation column / Anomalies dictionary covering all 5 producer event types / 3-scenario UI loadability / Copilot full-evidence display + `risk_if_ignored` + 4 metadata fields all shipped.
