...
description: Owns the release pipeline — version pins, `make archive`, RunSpace submission packaging, exiftool metadata strip, anonymity final gate.
tools: Read, Grep, Glob, Edit, Write, Bash(make archive), Bash(./verify.sh), Bash(scripts/check-no-secrets.sh*), Bash(exiftool *), Bash(git tag *), Bash(git log *)
model: sonnet
---
...
- **Package zip**: `make archive` invokes `git archive` — only tracked files are included, so `.git/` / `.venv/` / `node_modules/` / `tmp/` / `__pycache__/` / `.env*` / `.claude/settings.local.json` are automatically excluded (they're either gitignored or untracked).
