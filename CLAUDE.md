...
- **任何 PR**（含 docs-only）→ walk `docs/reviews/anti-pattern-checklist.md` 的 6 + 1 chain self-audit。每條結果寫進 PR body（`.github/pull_request_template.md` 已含欄位）；不適用必須標 `N/A — <理由>`，silent skip 等同這份 checklist 抓的 anti-pattern。
