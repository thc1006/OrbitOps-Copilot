...
- **`helm install` to a namespace that already has Kustomize-managed Services will FAIL** with `Service "copilot-api" in namespace "orbitops" exists and cannot be imported into the current release: invalid ownership metadata` (verified via `helm install ... --dry-run=server` against the live `orbitops` namespace). The chart cannot adopt resources it didn't create. To deploy the chart over a live Kustomize cluster, the operator must either (a) install to a fresh namespace, or (b) first `kubectl delete -k deploy/k8s/base` and then `helm install`. Documented as a deliberate trade-off — the alternative (unconditional Helm-adoption annotations) would let the chart silently take over Kustomize-owned resources, which is the wrong default. Future enhancement: an opt-in `--set adopt=true` value that injects helm-adoption annotations on demand.
- **Pod-selector label is shared with Kustomize.** Helm Service selectors use `app.kubernetes.io/name: <bare-svc-name>` — the same label set on Kustomize-managed Pods. If both deploy paths somehow coexist in one namespace (only possible after the failure above is bypassed via adoption flags), the chart's Services will route to *whichever Pod matches the label*, including Kustomize-deployed Pods. This is a feature for in-place migration (one Service routes to Pods from either source) but a footgun if the operator expects strict Helm-only ownership. Operators wanting strict isolation should add `app.kubernetes.io/managed-by: Helm` to the Service selector — out of scope for this ADR; tracked as a Sprint-3+ ergonomics item.
