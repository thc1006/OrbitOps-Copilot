...
| 1:10–1:30 | Copilot panel showing evidence JSON | "The copilot replies, citing `orbitops_beam_snr_db{beam_id='beam-1'}=6.5 dB`. Every response carries an evidence block (`metrics_used` + `logs_used` + `scenario_id` + `confidence`) plus structured analysis (`summary` / `likely_cause` / `recommended_actions[]` / `risk_if_ignored` / `unknowns`) — no hallucinations allowed (ADR-004)." |
