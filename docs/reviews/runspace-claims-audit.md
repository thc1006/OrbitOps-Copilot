...
| R4 | 08/0:20–0:40 | Stack lists "CesiumJS" as current | No `cesium*` package in `digital-twin-ui/package.json` `dependencies`/`devDependencies` (verified empty); UI uses pure SVG (`digital-twin-ui/README.md` "Why SVG, not CesiumJS" section); CLAUDE.md §9 + SPEC-004 §3 defer CesiumJS pass viz to VS-13 / P1 | **OVERCLAIM** | Moved to "P1 roadmap" callout in same line; current stack now reads "FastAPI, React + Material UI" |
...
| R6 | 08/1:10–1:30 | Evidence block keys = "metrics_used + recommended_actions + unknowns" | `recommended_actions` is part of *response analysis*, not the `evidence` object. v2 `Evidence` model fields (verified via `services/copilot-api/src/copilot_api/models.py`): `metrics_used` / `logs_used` / `scenario_id` / `time_window_seconds` / `timestamp` (5 fields). `confidence` / `unknowns` are top-level `CopilotResponse` fields, NOT evidence fields. | **CATEGORY ERROR** | Split into "evidence block" (5 fields) vs "top-level structured analysis" (summary / likely_cause / recommended_actions / risk_if_ignored / confidence / unknowns) with ADR-004 pointer |
